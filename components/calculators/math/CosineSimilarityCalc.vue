<template>
    <div>
        ...
        <CalcBtn :showClear="calculated" @click="calculate" @clear="clear">Calculate Similarity →</CalcBtn>

        <CalcOutput :show="calculated" title="Cosine Similarity">
            <CalcOutputCell label="Similarity Score" :value="result.score" />
...
const interpLabel = computed(() => {
    const s = parseFloat(result.value.score)
    if (isNaN(s)) return ''
    if (s >= 0.95)  return 'Nearly identical direction'
    if (s >= 0.7)   return 'Highly similar'
    if (s >= 0.4)   return 'Moderately similar'
    if (s >= 0.1)   return 'Weakly similar'
    if (s >= -0.1)  return 'Orthogonal (no similarity)'
    if (s >= -0.7)  return 'Weakly dissimilar'
...
const interpColor = computed(() => {
    ...
    if (s >= 0.7)  return '#ddd6ff'
    if (s >= 0.1)  return '#f5e642'
    if (s >= -0.1) return '#e0e0e0'
    return '#ffd6d6'
})
...
function calculate() {
    ...
    if (magA === 0 || magB === 0) { error.value = 'Vectors must be non-zero.'; return }

    const score = dot / (magA * magB)
    const angle = (Math.acos(Math.max(-1, Math.min(1, score))) * 180) / Math.PI
