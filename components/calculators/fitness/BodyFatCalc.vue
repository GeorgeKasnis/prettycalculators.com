    ...
    <div>
        <UiFormErrorContainer :errors="form.errors">
            <UiFormContainer title="Body Fat Calculator">
                ...
                <InputsTextInput type="number" aria-label="Waist circumference" placeholder="Waist circumference" v-model="form.waist" measurementUnit="Cm" />
                <InputsTextInput type="number" aria-label="Hip circumference" placeholder="Hip circumference" v-if="form.gender === 'female'" v-model="form.hip" measurementUnit="Cm" />
...
export default {
    ...
    methods: {
        calculate() {
            ...
            this.form.result = this.form.result.toFixed(2);
        },
        // clearEverything() {
